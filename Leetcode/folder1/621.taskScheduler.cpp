...
static bool sortBySec(vector<int> a1,vector<int> a2)
{
	return (a1[1]>a2[1]);
}
...
int leastInterval(vector<char>& tasks,int n)
{
	...
	sort(chstr.begin(),chstr.end(),sortBySec);

	cout<<" chstr char and values -- > "<<endl;
	for (i=0;i<chstr.size();i++)
		cout<<chstr[i][0]<<" , "<<chstr[i][1]<<endl;
	cout<<endl;
				...
				for (j=i;j<(i+m);j++)
				{
					...
					else
					{
						if (pos<chstr.size())
						{
							...
							vstor[p][0] = chstr[pos][0];
							vstor[p][1] = chstr[pos][1];
							pos++;
...
int main()
{
	...
	cout<<" jobs in the tasks vector--> "<<endl;
	for (i=0;i<tasks.size();i++)
		cout<<tasks[i]<<" ";
	cout<<endl;
	cout<<" cooldown period between two same type of tasks --> "<<n<<endl;
	cout<<endl;
