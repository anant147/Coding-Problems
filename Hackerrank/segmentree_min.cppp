...
int sum(int a[],int segtree[],int v,int al,int ar, int ql,int qr)
{
	if(al==ql && qr == ar)
	{
	...
	if(qr<=mid)
	{
		...
		int val = sum(a,segtree,v*2,al,mid,ql,qr); 
		return val;
	...
	else if((mid+1)<=ql)
	{
		...
		int val = sum(a,segtree,(v*2)+1,mid+1,ar,ql,qr);
		return val;
	}
	else
	{
       ...
       int val1 = sum(a,segtree,v*2,al,mid,ql,mid);
       
       int val2 = sum(a,segtree,v*2+1,mid+1,ar,mid+1,qr);
       int val = val1+val2;
...
int main()
{
  ...
  {
  	int res = sum(a,segtree,1,0,n-1,left[i],right[i]);
  ...
  for(int i=0;i<left.size();i++)
  {
  	int rest = sum(a,segtree,1,0,n-1,left[i],right[i]);
