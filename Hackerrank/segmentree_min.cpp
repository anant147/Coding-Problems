...
int minimum(int a[],int segtree[],int v,int al,int ar, int ql,int qr)
{
	if(al==ql && qr == ar)
	{
	...
	if(qr<=mid)
	{
		...
		int val = minimum(a,segtree,v*2,al,mid,ql,qr); 
		return val;
	...
	else if((mid+1)<=ql)
	{
		...
		int val = minimum(a,segtree,(v*2)+1,mid+1,ar,ql,qr);
		return val;
	}
	else
	{
       ...
       int val1 = minimum(a,segtree,v*2,al,mid,ql,mid);
       
       int val2 = minimum(a,segtree,v*2+1,mid+1,ar,mid+1,qr);
...
int main()
{
  ...
  {
  	int res = minimum(a,segtree,1,0,n-1,left[i],right[i]);
  ...
  for(int i=0;i<left.size();i++)
  {
  	int rest = minimum(a,segtree,1,0,n-1,left[i],right[i]);
